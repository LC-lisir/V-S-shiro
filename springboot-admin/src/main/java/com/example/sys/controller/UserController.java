...
//    根据token获取用户信息
    ...
    @GetMapping("getlogin")
    public Result<Map<String, Object>> getloginId(@RequestParam("token") String token) {
        Map<String, Object> data = userService.getLogin(token);
        if (data!=null){
            return Result.success(data);
        }
        return Result.fail(20004,"登录用户获取失败");
    }
